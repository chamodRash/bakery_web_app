...
import { fetchCartItems, addItemToCart, deleteItems, updateItemInCart } from '@/actions/cart';
...
export async function PUT(request: Request) {
  try {
    ...
    if (typeof updates.quantity !== 'number' || typeof updates.total !== 'number') {
      throw new Error('Invalid quantity or total');
    }

    const data = await updateItemInCart(Number(id), updates.quantity, updates.total);
    return new NextResponse(
      JSON.stringify(data),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
