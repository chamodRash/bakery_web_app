...
export async function adminRegister(values: z.infer<typeof RegisterSchema>) {
  const {
    data: { user: master },
  } = await supabase.auth.getUser();
  const masterPhone = master?.user_metadata?.user_phone;
  const validateFields = RegisterSchema.safeParse(values);
  if (!validateFields.success) return { error: "Inavalid Fields!" };

  if (!values.code) {
    const otp = await generateRegisterOTP(masterPhone);

    const isOTPsent = await sendRegisterOTP(masterPhone, otp);
  ...
  if (values.code) {
    ...
    await supabase
      .from("profiles")
      .update({ phoneverified: new Date(), role: "ADMIN" })
